...
RUN cp /etc/apt/sources.list /etc/apt/sources.list.bak && \
    ...
    apt -y update && \
    apt -y upgrade && \
    apt install -y default-jdk autoconf flex bison bc python3 wget make gcc vim git ninja-build libglib2.0-dev libpixman-1-dev pkg-config &&\
    ...
    wget https://github.com/verilator/verilator/archive/refs/tags/v$VERILATOR_VERSION.tar.gz && \
